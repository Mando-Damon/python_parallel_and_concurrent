...
Barron 和 Olivia 用他们的共享购物清单演示了关键区，其中递增一个值包含三个步骤：读取、修改和写回；这三步构成了一个关键区，需要由每个线程作为一个不可中断的动作来执行。  
